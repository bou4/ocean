...
def results_dir(directory_name : str) -> str:
    """Specifies the directory where the PSF files (results) are stored.

    If you do not specify a directory with this command, the PSFfiles are placed in ../psf to the netlist directory.

    Note: The directory you specify with resultsDir is also where the `simulator.out` file is created.

    Note: Some simulators are designed to always put their results in a specific location.
    For these simulators, resultsDir has no effect.
    ...
    Args:
        directory_name: Directory where the PSF files are to be stored.
    
    Returns:
        The results directory.
    """
    return ws['resultsDir'](directory_name)
...
def open_results(job_or_directory_name : Symbol | str = None, enable_calculator_expressions : bool = None) -> str:
    ...
    When :func:`open_results` passes a symbol, it interprets the value as a job name and opens the results for the specified job.
    A job name is defined when a :func:`run` is issued.

    When openResults passes a text string, it opens simulation results stored in PSF files in the specified directory.
    ...
    If you want to find out which results are currently open, you can use :func:`open_results` with no argument.
    The directory for the results that are currently open is returned.

    Note: If you run a successful simulation with distributed processing disabled, the results are automatically opened for you.
    Also, a job name is generated by every analysis, even if distributed processing is not enabled.

    Args:
        job_or_directory_name:
            Either the name of a distributed process job or the directory containing the PSF files.
    ...
    if job_or_directory_name is not None:
        args.append(job_or_directory_name)
    
    if enable_calculator_expressions is not None:
        args.append(enable_calculator_expressions)

    return ws['openResults'](*args)
